...
extension TestViewController : WKScriptMessageHandler{
    func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "sendResult"{
            ...
            let score = Double(truncating: dict["score"] as! NSNumber)
            print(score)
            if score < 0.3 {
                userDataDelegate?.changeCurrentUserStatus(status: BLADE_RUNNER)
            }
            if score > 0.7{
                userDataDelegate?.changeCurrentUserStatus(status: DANGEORUS_ANDROID)
            }
